...
title: Data Input
sidebar_position: 3
description: "Learn how to configure data input in the Eledo Make module by selecting templates, mapping fields, using functions, and handling repeatable data."
keywords:
  - Eledo Make data input
  - Make field mapping
  - dynamic input form
  - template schema
  - Make functions
...
First, select a template. Once selected, [Make](https://www.make.com/) retrieves the template schema and generates a dynamic input form that mirrors the template fields.

This is the main way to bind your data to template fields in Make.

![Make Dynamic Form.](/img/integrations/make/make-dynamic-form.png)

:::note

The Make integration currently lists only your private templates. Public templates must first be copied into your account before they can be used in a scenario.
...
![Make Static Value.](/img/integrations/make/make-static-value.png)
...
3. Insert a value from a previous module by double-clicking it or dragging it into the field

Make displays outputs from previous modules and allows you to map them directly to the selected template field.

:::note

Make sure a source module is already added and configured before mapping dynamic values. Otherwise, the data you want to insert will not be available yet.

:::

![Make Data Binding.](/img/integrations/make/make-data-binding.png)
...
![Make Functions in Mapping.](/img/integrations/make/make-functions.png)
...
![Make Complex Object.](/img/integrations/make/make-complex-object.png)
...
For example, numeric values are typically formatted in the template using [Eledo functions](/docs/product/template-editor/data-expressions/number-functions.mdx) rather than inside Make.
