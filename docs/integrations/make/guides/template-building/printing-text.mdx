---
title: Printing Text
sidebar_position: 2
description: "Learn how to print dynamic text in Eledo templates using Make data mapping, Text Box fields, and optional template expressions."
keywords:
  - Eledo Make printing text
  - Make text mapping
  - dynamic text
  - Text Box
  - template expressions
  - PDF templates
  - document automation
---

import DocFeedbackForm from '@site/src/components/DocFeedbackForm';

# Printing Text

This guide shows how to print dynamic text into your PDF using [Make](https://www.make.com/).

---

## Step 1 — Prepare your template

In the Eledo editor:

1. Click the [**Text Box (TXT)**](/docs/product/template-editor/components/text-box.mdx) component in the toolbar  
2. Place it into your document  
3. In **Data (expression)**, enter a field name (for example `ProductName`)  
4. Confirm and save the template  

![Text Box Configuration](/img/integrations/make/TextPrintingMake1.gif)

:::tip

Learn more about Text Box: [/docs/product/template-editor/components/text-box.mdx](/docs/product/template-editor/components/text-box.mdx)

:::

---

## Step 2 — Use the template in Make

In your Make scenario:

1. Add the **Eledo PDF** module  
2. Select your template  
3. Click **Refresh Custom Fields**

You will now see your field (for example `ProductName`) available for mapping.

---

## Step 3 — Map your data

1. Click into the field (for example `ProductName`)  
2. Select a value from your data source (Google Sheets, CRM, etc.)  
3. Confirm the mapping  

![Mapping Text Value](/img/integrations/make/TextPrintingMake2.gif)

---

## Step 4 — (Optional) Format the output

You can control how text appears directly in the template using expressions.

Examples:

```
"Product: " + ProductName
NUM(Price, 0, 2) + " €"
```

:::tip

Learn how to format values: [/docs/product/template-editor/guides/printing-text.mdx](/docs/product/template-editor/guides/printing-text.mdx)

:::

---

## Related guides

- Printing numbers → [Printing Number](./printing-number.mdx)  
- Printing dates → [Printing Date](./printing-date.mdx)  

---

## Result

When the scenario runs:

- data is passed from your source  
- Eledo fills the Text Box  
- the final document is generated  

---

## Next steps

- Build a full workflow → [Generate Your First PDF](../scenarios/generate-first-pdf.mdx)

<DocFeedbackForm />