---
title: Images in a 2-Column Layout
sidebar_position: 5
description: "Learn how to display monday.com file images in a two-column Eledo PDF layout using Block Repetition, indexed array access, and Dynamic Images."
keywords:
  - monday.com image layout
  - Eledo Dynamic Images
  - two-column layout
  - Block Repetition
  - file array
  - indexed access
  - PDF templates
---

import DocFeedbackForm from '@site/src/components/DocFeedbackForm';

# Images in a 2-Column Layout

To display images in two columns, use [Block Repetition](/docs/product/template-editor/components/block-repetition.mdx) together with indexed access to the file array.

---

## Block Repetition

Use the following data expression to iterate over the images in pairs:

```
FOR(1, SIZE(item.file_mknjndsx) / 2 + 1)
```

---

## Dynamic Images

Use indexed access to retrieve two images per row:

```
PROPERTY(GET(parent.item.file_mknjndsx, $index * 2 - 1), "public_url")
PROPERTY(GET(parent.item.file_mknjndsx, $index * 2), "public_url")
```

---

This approach ensures that each row contains up to two images.

<DocFeedbackForm />