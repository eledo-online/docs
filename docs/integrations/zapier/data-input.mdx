...
title: Data Input
sidebar_position: 3
description: "Learn how to configure data input in the Eledo Zapier step by selecting templates, mapping fields, using static values, and handling limitations."
keywords:
  - Eledo Zapier data input
...
The Zapier integration currently lists only your private templates. Public templates must first be copied into your account before they can be used in a Zap.
...
In Zapier, all input values are treated as text. Whether the original value is a number, date, or string, it is passed to Eledo as text.
...
Make sure a data source step (such as Google Sheets) is properly added and configured before mapping dynamic values. Otherwise, Zapier will prompt you to configure one.
