...
Define an array field name or formula using the [expression language](../template-building-guides/expressions.mdx) to be evaluated as an array of items. The selected block will be repeated based on the array items count, and each block's data context will be the corresponding item.
