---
title: Introduction
sidebar_position: 1
description: "Learn what Eledo is, how its template-based PDF generation model works, and where to start with templates, APIs, and integrations."
keywords:
  - Eledo
  - document automation
  - PDF generation
  - template editor
  - structured data
  - document templates
  - automation platform
---

import DocFeedbackForm from '@site/src/components/DocFeedbackForm';

# Eledo

Eledo is a document automation platform for generating PDF documents from templates and structured data.

It is designed for developers and teams who want reliable document generation without maintaining their own rendering infrastructure.

---

## Core concept

Eledo is based on a simple model:

1. **Template** — defines layout and logic  
2. **Input data** — provides values  
3. **Output PDF** — generated document  

Everything in the system builds on this foundation.

---

## How it is used

Eledo supports two primary usage patterns:

### [Template Editor](./template-editor/index.mdx)

- [design document layout](./template-editor/guides/visual-template-design.mdx)
- define dynamic content using [expressions](./template-editor/data-expressions/index.mdx)
- build reusable templates  

---

### [API](../api/index.mdx) and [integrations](../integrations/index.mdx)

- generate documents programmatically  
- integrate with existing systems  
- automate workflows  

---

## Design principles

Eledo follows a few key principles:

- **Clear APIs over hidden behavior**  
- **Automation over manual work**  
- **Deterministic output over implicit logic**  
- **Integrations over closed ecosystems**  

---

## What Eledo is not

Eledo is not:

- a document editor with manual workflows  
- a storage system for generated files  
- a full document management platform  

Its focus is **generation and automation**, not lifecycle management.

---

## Where to start

- Learn how templates work → [Template Editor](./template-editor/index.mdx)  
- Explore components → [Components](./template-editor/components/index.mdx)  
- Understand expressions → [Data Expressions](./template-editor/data-expressions/index.mdx)  
- Integrate via API → [API](../api/index.mdx)

---

<DocFeedbackForm />