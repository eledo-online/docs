---
title: Table
sidebar_position: 3
description: "Use Table in the Eledo Template Editor to structure PDF content into rows and columns for invoices, item lists, and data-driven layouts."
keywords:
  - Eledo Table
  - PDF table
  - document layout
  - structured data
  - invoice tables
  - dynamic rows
  - PDF templates
---

import DocFeedbackForm from '@site/src/components/DocFeedbackForm';
import ImageWithCaption from '@site/src/components/ImageWithCaption';

# Table

A Table allows you to structure content into rows and columns.

It is commonly used for layouts such as invoices, item lists, or structured data sections.

---

## Add a Table

To insert a table into your document:

1. Place the cursor where you want the table to appear  
2. Click the **Table** icon in the toolbar  
3. Select the number of rows and columns  
4. Confirm to insert the table  

The table will be inserted at the cursor position.

<ImageWithCaption
  src="/img/product/template-editor/document-elements/table-grid.png"
  alt="Table selection grid"
  caption="Select the table size using the grid."
/>

---

## Editing a Table

Once inserted, you can:

- add or remove rows and columns  
- merge or split cells  
- adjust layout and formatting  
- insert components inside cells  

Tables behave like structured containers for content.

---

## Using Tables with Dynamic Content

Tables are often used together with dynamic components:

- [**Block Repetition**](../components/block-repetition.mdx) — repeat rows for lists (e.g. invoice items)  
- [**Text Box**](../components/text-box.mdx) — display values inside cells  
- [**Dynamic Image**](../components/dynamic-image.mdx) — insert images into cells  

This makes tables essential for building structured, data-driven documents.

---

## When to use Table

Use a Table when you need:

- structured layout (rows and columns)  
- repeating data (lists, items)  
- precise alignment of content  

---

## Next steps

- Repeat rows dynamically → [Block Repetition](../components/block-repetition.mdx)
- Insert dynamic values → [Text Box](../components/text-box.mdx)
- Learn how layout works → [Page Break](./page-break.mdx)

<DocFeedbackForm />